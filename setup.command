...
if [[ "$OSTYPE" == "darwin"* && -x "$(command -v sips)" ]]; then
  ...
  sips -Z 1080 $SCRIPT_PATH/photos/**/*.min.jpeg &>/dev/null
  sips -Z 1080 $SCRIPT_PATH/photos/**/*.min.png &>/dev/null
  sips -Z 1080 $SCRIPT_PATH/photos/**/*.min.jpg &>/dev/null
...
if [ -n "$(uname -a | grep Ubuntu)" -a -x "$(command -v mogrify)" ]; then
  ...
  # low res version of image
  python $SCRIPT_PATH/tools/duplicate.py min
  mogrify -resize 1080x $SCRIPT_PATH/photos/**/*.min.jpeg &>/dev/null
  mogrify -resize 1080x $SCRIPT_PATH/photos/**/*.min.png &>/dev/null
  mogrify -resize 1080x $SCRIPT_PATH/photos/**/*.min.jpg &>/dev/null
